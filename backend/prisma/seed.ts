  ...
  async function main() {
    ...
    if (!user) throw new Error('No user found. Register first, then run seed.');
    ...
    for (const q of allQuests) {
      const quest = await prisma.quest.upsert({
        where: { userId_key: { userId: user.id, key: q.key } },
        update: {
          name: q.name,
          icon: q.icon,
          sub: q.sub,
      ...
      for (const t of q.tasks) {
        await prisma.task.upsert({
          where: { questId_key: { questId: quest.id, key: t.key } },
          update: {
            name: t.name,
            note: t.note,
            exp: t.exp,
